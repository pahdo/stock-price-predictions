...
def preprocess_texts(tr, te, part_num, split='train'):
    ...
    """
    gen1, gen2 = itertools.tee(utils_v2.load_texts(my_config.source_dir, split, tr, te))
    ...
    """
    for doc in nlp.pipe(texts, batch_size=1000, n_threads=64): # yields Doc objects in order
...
#        os.makedirs(os.path.dirname(output_file)) # repeated calls take ~ 0.00029206275939941406 seconds each => 2.9 seconds per 10k docs
...
def prepare_dataset(tr, te, part_num, split='train'):
    ...
    """
#    gen = utils_v2.load_data(my_config.data_dir, split, my_config.train_quarters, my_config.test_quarters, yield_paths=True) 
    gen = utils_v2.load_data(my_config.data_dir, split, tr, te, yield_paths=True) 
