...
def read_dataset(label_horizon, subset='full', doc2vec=False):
    ...
    with open(data_index_path, 'r') as data_index:
        for path_prices_labels in data_index.readlines():
            ...
            labels = labels.split(',')
            if doc2vec:
                yield pickle.loads(codecs.decode(v.encode(), 'base64')), [float(p) for p in prices], bin_alpha(float(labels[label_horizon]))
            else:
                with open(path, 'r') as t:
                    yield t.read(), [float(p) for p in prices], bin_alpha(float(labels[label_horizon]))
